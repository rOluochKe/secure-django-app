...
If you're following the Web Application Security, read the file **RELEASENOTES.md** for changes that have happened since the app was written.  Browser features may have changed since the app was written, as well as third-party software components such as Virtualbox.
...
- My upcoming Web Application Security
- My web application security practice.

The repo contains two VMs for performing hands-on exercises.  It is not a real coffee shop and is not intended for any purpose other than using with the practices.
...
`git clone git@github.com:roluochke/secure-django-app.git`

or

`git clone https://github.com/roluochke/secure-django-app.git`

Build the two VMs
-----------------

The `secure-django-app` directory contains two subdirectories: `coffeeshop` and `csthirdparty`.

Go to the `vagrant` directory of the `coffeeshop` directory and build the VM using Vagrant:

`cd secure-django-app/coffeeshop/vagrant`
...
```
cd secure-django-app/csthirdparty/vagrant
vagrant ssh
```
...
If you're following the app, you are now ready to start the hands-on exercises.
